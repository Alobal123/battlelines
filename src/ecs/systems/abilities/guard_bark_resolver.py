...
class GuardBarkResolver(AbilityResolver):
    """Resolver for the Guard Bark ability that disrupts witchfire tiles."""
    ...
    def resolve(self, ctx: AbilityContext) -> None:  # pragma: no cover - behaviour tested via integration
        ...
        ctx.scratchpad["guard_bark_positions"] = [list(pos) for pos in positions]
        ctx.scratchpad["guard_bark_target_types"] = target_types
    ...
    def _spawnable_mana_types(world: World) -> Sequence[str]:
        registry_entity = GuardBarkResolver._registry_entity(world)
