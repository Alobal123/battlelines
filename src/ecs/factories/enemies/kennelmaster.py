...
DEFAULT_KENNELMASTER_LOADOUT: Sequence[str] = ()


def create_enemy_kennelmaster(
    world: World,
    *,
    ability_names: Iterable[str] | None = None,
    max_hp: int = 28,
) -> int:
    """Spawn the Kennelmaster enemy."""

    loadout = tuple(ability_names) if ability_names is not None else DEFAULT_KENNELMASTER_LOADOUT
    ...
    enemy_entity = world.create_entity(
        ...
        Affinity(base={"beast": 2}),
        Character(
            slug="kennelmaster",
            name="Kennelmaster",
            description="A stern handler commanding spectral hounds.",
            portrait_path="kennelmaster.png",
        ),
    )
